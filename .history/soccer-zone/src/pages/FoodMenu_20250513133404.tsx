...
import Footer from "../components/Footer";
import FoodList from "../components/FoodList";
import Bg from "../assets/svg/bg.png"create a different page where the food details will show when clicked for spring rolls and chicken wings
...
const FoodMenu = () => {
  ...
  const [selectedFood, setSelectedFood] = useState<null | {name: string; description: string; price: number}>(null);
  ...
  return (
   <div className="bg-secondary-d" style={{ backgroundImage: `url(${Bg})` }}>
    ...
    >
      ...
      {selectedFood && (
        <div className="fixed top-20 right-20 bg-white p-6 rounded-lg shadow-lg w-[400px] z-50">
          <h3 className="text-2xl font-bold mb-4">{selectedFood.name}</h3>
          <p className="mb-4">{selectedFood.description}</p>
          <p className="font-mono font-bold">₦{selectedFood.price.toLocaleString()}</p>
          <button
