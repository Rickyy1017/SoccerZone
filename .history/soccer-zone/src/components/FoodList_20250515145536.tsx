...
const FoodList: React.FC<FoodListItemProps> = ({ name, description, details, price}) => {
  ...
  return (
    <div className="flex justify-between border-b border-dashed border-[#006837] p-4 my-6">
      <div>
        <div onClick={handleClick} className="font-semibold text-[25px] capitalize py-[10px] cursor-pointer">
          {name}
        </div>
        <div className="text-[15px] border max-w-full text-primary">{description}</div>
      </div>
